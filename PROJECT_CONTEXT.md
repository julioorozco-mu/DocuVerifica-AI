...
* Estado actual: Supabase local se levanta con Supabase CLI (`supabase start`) y usa GoTrue/Auth real. El backend conserva un fallback temporal de JWT propio solo para no bloquear entornos que aún no exportaron `SUPABASE_ANON_KEY`.
...
* Redis
* RQ
* Estado actual Fase 1/2: Redis permanece preparado para Fase 6, pero no se usa y no publica puerto a la red/VPN.
